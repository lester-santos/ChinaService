...
namespace MaxiFlexProxy
{
    ...
    public class MaxiFlexData : IMaxiFlexData
    {
        ...
        {
            return _BusinessLogic.DownloadCardDesign(filename);
        }

        public bool SetSecurityQuestion(string secQuestions)
        {
            return _BusinessLogic.SetSecurityQuestion(secQuestions);
        }

        public string GetSecurityQuestion(string IDNo)
        {
            return _BusinessLogic.GetSecurityQuestion(IDNo);
        }

        public bool ChangeCardNo(string IDNo)
        {
            return _BusinessLogic.ChangeCardNo(IDNo);
        }

        public byte[] byteLOApdf(string ClaimNo, string UserID)
        {
            App_Code.DataAccessLayer _DAL = new App_Code.DataAccessLayer();
            DataSet ds = _DAL.GetLOAInformation(ClaimNo);
            string Coverage = ds.Tables[0].Rows[0]["Coverage"].ToString();
            byte[] byteLOA = null;
            LOApdfGenerator LOA = new LOApdfGenerator();
            if (Coverage == "I")
            {
                byteLOA = LOA.byteLOApdf(ClaimNo, ds);
            }
            else if (Coverage == "OC")
            {
                byteLOA = LOA.DownloadOPConLoaPdfReport(ClaimNo, ds);
            }
            else if (Coverage == "E" || Coverage == "OL")
            {
                byteLOA = LOA.DownloadOPLabLoaPdfReport(ClaimNo, ds);
            }
            return byteLOA;
        }
        ...
        {
            return _BusinessLogic.downloadImage(filename);
        }

        public bool uploadImage(string _fileName, byte[] img)
        {
            return _BusinessLogic.uploadImage(_fileName, img);
        }
